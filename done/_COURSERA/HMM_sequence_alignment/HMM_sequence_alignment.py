...
def fulfillWithPath(path):
    path_ = []
    for i in xrange(0, len(path) - 1):
        path_.append(path[i])
        if hasGap(path[i], path[i + 1]):
            path_ += fillingElements(path[i], path[i + 1])
    path_.append(path[-1])
    return path_

def hasGap(str1, str2):
    index1 = int(filter(str.isdigit, str1))
    nextIndex = int(filter(str.isdigit, str2))
    if nextIndex - index1 > 1:
        return True
    return False

def fillingElements(str1, str2):
    index1 = int(filter(str.isdigit, str1))
    nextIndex = int(filter(str.isdigit, str2))
    fillers = []
    for i in xrange((index1 + 1), nextIndex):
        strIn = "D" + str(i)
        fillers.append(strIn)
    return fillers


def profileHMM(threshold, alphabet, Alignment, theta):
    ...
    states = [n.base for n in headRow]

    paths = []; letters = { }
    for line in Alignment:
        path = alignmentPath(line, columsToRemove, nodes, letters)
        paths.append(path)
    addInserionNodes(letters, nodes, theta)

    TransitionMatrix = transitionWithPathsAndGraph(connections, nodes, paths, theta)
    EmmiterMatrix = emmitionWithLettersAndGraph(nodes, alphabet, letters, theta)
    return TransitionMatrix, EmmiterMatrix, states

def addInserionNodes(letters, nodes, theta):
    insertionItems = nodes.inserts
    for insertionItem in insertionItems:
        if insertionItem not in letters:
            letters[insertionItem] = []
...
def emmitionWithLettersAndGraph(nodes, alphabet, letters, theta):
    headRow = rowHeadWithNodes(nodes)
    EmitionMatrix = generateEmitionMatrix(headRow, letters, alphabet, theta)
    return emittionMatrixToPrettyTable(EmitionMatrix, headRow, alphabet)

def generateEmitionMatrix(headRow, letters, alphabet, theta):
    EmitionMatrix = []
    ...
    for node in headRow:
        ...
        Emmition = [x + theta for  x in Emmition]; sumI = sum(Emmition)
        Emmition = [x / sumI for x in Emmition]
        EmitionMatrix.append(Emmition)
...
def transitionWithPathsAndGraph(connections, nodes, paths, theta):
    end = nodes.end
    for path in paths:
        for idx, node in enumerate(path):
            ...
            edge.weight += 1
    headRow = rowHeadWithNodes(nodes)
    Transitions = generateTransitionTable(headRow, connections, theta)
...
def generateTransitionTable(headRow, connections, theta):
    TransitionMatrix = []
    for i in xrange(0, len(headRow)):
        ...
        #pseudocounts
        for k in validRows:
            TransitionTableRow[k] += theta
        sumI = sum(TransitionTableRow)
        if sumI > 0:
            TransitionTableRow = [x / sumI for x in TransitionTableRow]
